...
export const Question = () => {
  ...
  const groupedQuestions = useMemo(() => {
    return Array.from(
      { length: Math.ceil(questions.length / 5) },
      (_, i) => questions.slice(i * 5, (i + 1) * 5)
    );
  }, []);
  ...
  return (
    ...
    <div className="w-10/12 md:w-80 lg:w-1/3 rounded-3xl p-10 px-14 lg:px-14 bg-pink-normal capitalize font-semibold text-2xl lg:text-4xl -my-20 lg:mt-0 z-10">
      <p>get ready to</p>
      <p>watch them shine</p>
    </div>

    <div className="w-full p-spacing-normal pt-40 flex flex-col lg:rounded-3xl bg-yellow-normal">
      <p className="text-3xl mb-40">
        Kidpreneur Bootcamp: equipping young minds to explore, create, and grow
          ...
          {groupedQuestions.map((group, groupIndex) => (
            ...
            >
              {group.map((item, index) => (
                <div key={index} className="flex flex-col">
                  <p className="">
                    {(groupIndex * 5) + index + 1} Q: {item.quesiton}
                  </p>
                  <p className="">
                    A: <span dangerouslySetInnerHTML={{ __html: item.answer }} />
                  </p>
                </div>
              ))}
            </div>
